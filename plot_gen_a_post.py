...
def drawPlot(algId, dataPoints, specs):
    ...
    if regression:
        ...
        if doPlotCIs:
            ...
            if algId == algListGMP.id_:
                maxN = np.amax(validX)
                popt, pcov = curve_fit(funcExp, validX, validY, bounds=[0,maxN])
...
if __name__ == '__main__':
    ...
    graphSizes = ['sm', 'md', 'lg']
    algIds = [algListGMP.id_, algListCIBF.id_, algListCI.id_]
    # prefixes = ['_ci', '_bf', '_gmp']
    # basePath = 'experiments/a_bnlearn/mac
    basePath = 'experiments/post_submission/exp_a'
    # basePath = 'experiments/pre_submission/exp_a'
    extension = '.csv'
    ...
    for algId in algIds:
        ...
        # if algId == algListGMP.id_:
        #     graphSizesToCheck = ['sm']
        # elif algId == algListCIBF.id_:
        #     graphSizesToCheck = ['sm', 'md']
        # else:
        #     graphSizesToCheck = graphSizes
        graphSizesToCheck = graphSizes

        for size in graphSizesToCheck:
            currentPath = basePath + '/' + algId + '/' + size + '/'
            
            for instanceName in graphNames[size]:
                filePath = currentPath + instanceName + extension
